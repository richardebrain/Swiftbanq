        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-3">
              ...
              {s < 2 && <div className={`w-12 h-0.5 ${step > s ? 'bg-brand-dark' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-8 md:p-10 shadow-sm">
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-bold text-brand-dark mb-3">
                  {form.service === 'treasury' ? 'Investment amount' : 'Loan amount needed'} <span className="text-red-500">*</span>
                </label>
